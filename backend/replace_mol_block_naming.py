...
for npe in new_npmrd_entries:
    try:
        ...
        for data_entry in data:
            ...
            if "contains_unassigned_stereochemistry" not in data_entry or data_entry["contains_unassigned_stereochemistry"] is None:
                data_entry['contains_unassigned_stereochemistry'] = unassigned_stereochemistry
            
            if "mol_block_type" in data_entry:
                data_entry["canonicalized_mol_block_dimensionality"] = data_entry.pop("mol_block_type")
                modified = True
            
            if "mol_block" in data_entry:
                data_entry["canonicalized_mol_block"] = data_entry.pop("mol_block")
                modified = True
            
            if len(data_entry['h_nmr']['spectrum']) > 0:
                if "rdkit_index" in data_entry['h_nmr']['spectrum'][0]:
                    data_entry['h_nmr']['has_index_assignments'] = True
                else:
                    data_entry['h_nmr']['has_index_assignments'] = False
            else:
                data_entry['h_nmr']['has_index_assignments'] = None
            
            if len(data_entry['c_nmr']['spectrum']) > 0:
                if "rdkit_index" in data_entry['c_nmr']['spectrum'][0]:
                    data_entry['c_nmr']['has_index_assignments'] = True
                else:
                    data_entry['c_nmr']['has_index_assignments'] = False
            else:
                data_entry['c_nmr']['has_index_assignments'] = None
